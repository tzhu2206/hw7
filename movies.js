...
firebase.auth().onAuthStateChanged(async function(user) {
    if (user) {
        ...
        let apiKey = 'c7ad6f6cf7e93c1e35ff95693e0fdd3f'
        ...
        document.querySelector('.sign-in-or-sign-out').innerHTML = `
            <p class="text-white text-2xl font-bold p-4 welcome">Welcome, ${currentUser.displayName}!</p>
            <button class="text-white text-xl underline sign-out">Sign Out</button>
        `
        ...
        for (let i=0; i<movies.length; i++) {
            ...
            document.querySelector(`.movie-${movie.id}`).addEventListener('click', async function(event) {
                ...
                if (movieElement.classList.contains('opacity-20')){
                    ...
                    await db.collection('watched').doc(`${movies[i].id}-${user.uid}`).delete()
                } else {
                    movieElement.classList.add('opacity-20')
                    await db.collection('watched').doc(`${movies[i].id}-${user.uid}`).set({})
                }
            }) 
        }
    } else {
